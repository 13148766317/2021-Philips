...
- (void)setupUI{
    ...
    //添加新用户标题
    UILabel *addNewTitleLabel = [UILabel new];
    addNewTitleLabel.text = Localized(@"DeviceShare_Add_NewUser");
    addNewTitleLabel.textColor = [UIColor blackColor];
    addNewTitleLabel.textAlignment = NSTextAlignmentLeft;
    addNewTitleLabel.font = [UIFont boldSystemFontOfSize:17];
    [headerView addSubview:addNewTitleLabel];
    [addNewTitleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    //已分享用户背景View
    UIView *ownBackView = [UIView new];
    ownBackView.backgroundColor = KDSRGBColor(0xf2, 0xf2, 0xf2);
    [headerView addSubview:ownBackView];
    [ownBackView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    //已添加新用户标题
    UILabel *ownTitleLabel = [UILabel new];
    ownTitleLabel.text = Localized(@"DeviceShare_HasShare_User");
    ownTitleLabel.textColor = [UIColor blackColor];
    ownTitleLabel.textAlignment = NSTextAlignmentLeft;
    ownTitleLabel.font = [UIFont systemFontOfSize:15];
    [ownBackView addSubview:ownTitleLabel];
    [ownTitleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(headerView).offset(15);
        make.top.equalTo(ownBackView).offset(10);
        make.size.mas_offset(CGSizeMake(120,20));
    }];
    ...
    [self.tableView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_offset(CGSizeMake(kScreenWidth,kScreenHeight-15));
    }];
    
    switch (self.keyType)
    {
        case KDSBleKeyTypeReserved://保留值
            self.navigationTitleLabel.text = Localized(@"DeviceShare_HeadTitle");
            //[addBtn setTitle:Localized(@"shareUser") forState:UIControlStateNormal];
            //flagIV.image = [UIImage imageNamed:@"member"];
            break;
            
        case KDSBleKeyTypePIN://PIN(personal indentifier number)码，相当于开门密码，6~12位
        ...
        case KDSBleKeyTypeRFID://RFID卡片。4~10位。
...
- (void)clickSyncBtnAction:(UIButton *)sender
{
    ...
    self.receipt = [self.lock.bleTool getAllUsersWithKeyType:self.keyType completion:^(KDSBleError error, NSArray<KDSBleUserType *> * _Nullable users) {
        ...
        {
            [hud hideAnimated:YES];
            [MBProgressHUD showError:Localized(@"Network_Loading_Error")];
        }
    }];
}
...
- (void)clickAddBtnAction:(UIButton *)sender
{
    ...
    }else
    {
        ...
        [self getAllKeys:self.keyType times:0 completion:^(KDSBleError error, NSArray<KDSBleUserType *> * _Nullable users) {
            ...
            if (error != KDSBleErrorSuccess)
            {
                [MBProgressHUD showError:Localized(@"Loading_Lock_InformationError")];
                return;
            }
            BOOL upperLimit = NO;
            BOOL usersCount = NO;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.catEye) {
        ...
        cell.name = member.userNickname ?: member.uname; //[KDSTool showNameStr:member.userNickname Length:10] ?: [KDSTool showNameStr:member.uname Length:10];
    }else{
        if (self.keyType == KDSBleKeyTypeReserved)
        {//网关锁
            KDSAuthMember *member = self.dataArr[indexPath.row];
        ...
        else
        {
            ...
            if ([m.nickName isEqualToString:m.num]) {
    ...
    if (!_footerView){
        ...
        switch (self.keyType)
        {
            case KDSBleKeyTypeReserved:
                label.text = Localized(@"DeviceShare_No_ShareUser");
                break;
