...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell.title isEqualToString:@"设备名称"]) {
        ...
        //   cell.subtitle = @"左开门";
        NSInteger opendir = self.lock.wifiDevice.openDirection.intValue;
        NSString *openDirStr = (opendir == 1 ? @"左开门" : @"右开门");
        if (opendir == 1) {
            openDirStr = @"左开门";
        }else if(opendir == 2){
            openDirStr = @"右开门";
        ...
        cell.hideSwitch = YES;
        cell.hideArrow = YES; // 目前只是显示  将箭头去掉
    } else if ([cell.title isEqualToString:@"开门力量"]) {
        //cell.subtitle = @"低扭力";
        NSInteger openforce = self.lock.wifiDevice.openForce.intValue;
        ...
        // cell.subtitle = @"15秒自动";
        NSInteger lockingway = self.lock.wifiDevice.lockingMethod.intValue;
        NSString *openwayStr;
        switch (lockingway) {
            case 1:
                openwayStr = @"自动上锁";
...
//        KDSSetDoorDirectionViewController *doorDir = [KDSSetDoorDirectionViewController new];
//        doorDir.lock = self.lock;
//        doorDir.myBlock = ^(NSString *myText){
//            cell.subtitle = myText;
//            NSLog(@"zhu 设置后返回的值==%@",myText);
//            };
//        [self.navigationController pushViewController:doorDir animated:YES];
    } else if ([cell.title isEqualToString:@"开门力量"]) {
//        KDSSetDoorStrengthViewController *doorStr = [KDSSetDoorStrengthViewController new];
//          doorStr.lock = self.lock;
//         doorStr.myBlock = ^(NSString *myText){
//            cell.subtitle = myText;
//            NSLog(@"zhu 设置后返回的值==%@",myText);
//            };
//        [self.navigationController pushViewController:doorStr animated:YES];
    } else if ([cell.title isEqualToString:@"上锁方式"]) {
//        KDSLockedWayViewController *doorWay = [KDSLockedWayViewController new];
//         doorWay.lock = self.lock;
//         doorWay.myBlock = ^(NSString *myText){
//           cell.subtitle = myText;
//           NSLog(@"zhu 设置后返回的值==%@",myText);
//           };
//
//        [self.navigationController pushViewController:doorWay animated:YES];
    }
}
