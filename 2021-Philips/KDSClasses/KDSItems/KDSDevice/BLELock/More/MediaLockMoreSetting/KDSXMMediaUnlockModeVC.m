//
//  KDSXMMediaUnlockModeVC.m
//  2021-Philips
//
//  Created by zhaoxueping on 2020/10/10.
...
@interface KDSXMMediaUnlockModeVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation KDSXMMediaUnlockModeVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.navigationTitleLabel.text = Localized(@"DevicesDetailSetting_Verification_Mdel");
    [self setDataArray];
    [self setUI];
}
...
- (void)setUI
{
    self.currentIndex = self.lock.wifiDevice.safeMode.intValue;
    self.safeModeRawData = self.lock.wifiDevice.safeMode.intValue;
...
-(void)setDataArray
{
    ...
    [_dataSourceArr addObject:Localized(@"DevicesDetailSetting_Normal_Type")];
    [_dataSourceArr addObject:Localized(@"DevicesDetailSetting_PwdAndFingerprint")];
}
...
-(void)navBackClick
{
    ...
    [XMP2PManager sharedXMP2PManager].XMMQTTConnectDevStateBlock = ^(BOOL isCanBeDistributed) {
        if (isCanBeDistributed == YES) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                NSLog(@"MQTT 登录成功");
                [[KDSMQTTManager sharedManager] setLockSafaModeWithWf:self.lock.wifiDevice safeMode:(int)self.currentIndex completion:^(NSError * _Nullable error, BOOL success) {
                    [hud hideAnimated:YES];
                    if (success) {
                        [MBProgressHUD showSuccess:Localized(@"UnlockModel_Setting_Sucessful")];
                        self.lock.wifiDevice.safeMode = [NSString stringWithFormat:@"%d",(int)self.currentIndex];
                    }else{
