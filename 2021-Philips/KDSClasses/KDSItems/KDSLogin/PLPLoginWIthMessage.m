...
@implementation PLPLoginWIthMessage
   ...
   _chooseCountry  = [UIButton new];
    ...
    _signupBtn =  [UIButton new];
     ...
     [_signupBtn  mas_makeConstraints:^(MASConstraintMaker *make) {
         make.top.equalTo(verView.mas_bottom).offset(55);
         make.left.equalTo(self.view).offset(15);
         make.right.equalTo(self.view).offset(-15);
         make.height.equalTo(@44);
     }];
...
- (void)signupBtn:(id)sender {
    ...
    [[KDSHttpManager  sharedManager]   getUserByTokens:username tokens:self.codeTextField.text success:^(KDSUser * _Nonnull user) {
        KDSLog(@"kaadas 短信登录获取到的验证码 ==%@",user.uid);
        KDSLog(@"kaadas 短信登录获取到的验证码 ==%@",user.token);
