...
//从服务器获取到的绑定的微信的电话号码
@property (strong, nonatomic)  NSString *WXPhoneNum;
@end

@implementation PLPLoginWithWieiXin
...
- (void)setupUI {
    ...
    [_signupBtn setTitle:Localized(@"登录") forState:UIControlStateNormal];
    [_signupBtn setBackgroundColor:KDSRGBColor(0, 102, 161)];
    ...
    [_signupBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(verView.mas_bottom).offset(65);
        make.left.equalTo(self.view).offset(15);
        make.right.equalTo(self.view).offset(-15);
        make.height.equalTo(@44);
    }];
    ...
    [holderView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_signupBtn.mas_bottom).offset(12);
        make.centerX.equalTo(self.view.mas_centerX);
    ...
    [funView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.equalTo(@40);
        make.top.equalTo(_signupBtn.mas_bottom).offset(52);
        make.centerX.equalTo(self.view.mas_centerX).offset(-60);
    }];
    UIImageView *funimg = [UIImageView new];
    funimg.image = [UIImage imageNamed:@"philips_login_icon_iphone"];
    ...
    funlabel.text = Localized(@"Login_Phone_Code");
...
- (void)signupBtn:(id)sender {
    ...
    if (!openid && (_IsAppleID ==NO))  {
        return;
    }
    NSString  *  appleID = [KDSTool getDefaultLoginApple];
    if (!appleID && (_IsAppleID ==NO)) {
        return;
    }
    if (appleID) {
        [[KDSHttpManager sharedManager] putUserByAppleId:appleID tel:username tokens:self.codeTextField.text success:^(KDSUser * _Nonnull user) {
            [hud hideAnimated:YES];
            [MBProgressHUD showSuccess:@"绑定成功"];
            NSLog(@"userid==%@", user.uid);
            NSString *account = [KDSTool getDefaultLoginAccount];
            if (![account isEqualToString:self.userNameTextField.text]) {
                [[KDSDBManager sharedManager] resetDatabase];
            }
            [KDSTool setDefaultLoginAccount:self.userNameTextField.text];
            ...
            [UIApplication sharedApplication].keyWindow.rootViewController = tab;
                    ...
                    [hud hideAnimated:YES];
                    [TRCustomAlert showMessage:@"绑定失败" image:nil];
                }];
    }else{
        [[KDSHttpManager sharedManager] putUserByOpenId:openid tel:username tokens:self.codeTextField.text success:^(KDSUser *_Nonnull user) {
            [hud hideAnimated:YES];
            [MBProgressHUD showSuccess:@"绑定成功"];
            NSLog(@"userid==%@", user.uid);
            NSString *account = [KDSTool getDefaultLoginAccount];
