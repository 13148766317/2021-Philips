...
- (void)scrollIndicateView {
    ...
    [UIView animateWithDuration:YH_Duration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        if (weakSelf.yh_indicateStyle == YHSegementIndicateStyleDefault) {
            weakSelf.indicateView.frame = CGRectMake(CGRectGetMinX(weakSelf.selectedButton.frame) + weakSelf.buttonSpace, CGRectGetMinY(weakSelf.indicateView.frame), 20, YH_IndicateHeight);
        } else {
            //[self widthAtIndex:index]  存放按钮的宽度
            weakSelf.indicateView.frame = CGRectMake(CGRectGetMinX(weakSelf.selectedButton.frame), CGRectGetMinY(weakSelf.indicateView.frame), 20, YH_IndicateHeight);
        }
