//
//  PLPVoiceSettingViewController.m
//  2021-Philips
//
...
@interface PLPVoiceSettingViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation PLPVoiceSettingViewController
...
- (void)setUI{
    ...
    self.navigationTitleLabel.text = @"语音设置";
    
    self.currentIndex = self.lock.wifiDevice.volLevel.intValue;
    self.volLevelDataModel = self.lock.wifiDevice.volLevel.intValue;
...
-(void)navBackClick
{
    ...
    [XMP2PManager sharedXMP2PManager].XMMQTTConnectDevStateBlock = ^(BOOL isCanBeDistributed) {
        if (isCanBeDistributed == YES) {
            //讯美登录MQTT成功
            dispatch_async(dispatch_get_main_queue(), ^{
                [[KDSMQTTManager sharedManager] setLockVoiceLevelWithWf:self.lock.wifiDevice voiceLevel:(int)self.currentIndex completion:^(NSError * _Nullable error, BOOL success) {
                    [hud hideAnimated:YES];
                    if (success) {
                        [MBProgressHUD showSuccess:Localized(@"Setting_Status_Successful")];
                        self.lock.wifiDevice.volLevel = [NSString stringWithFormat:@"%d",(int)self.currentIndex];
                    }else{
...
-(NSMutableArray *)dataSourceArr{
    if (!_dataSourceArr) {
        _dataSourceArr = [NSMutableArray arrayWithObjects:@"静音",@"低音量",@"高音量", nil];
    }
