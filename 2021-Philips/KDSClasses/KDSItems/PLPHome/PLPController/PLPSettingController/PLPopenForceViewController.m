...
@interface PLPopenForceViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation PLPopenForceViewController
...
- (void)setUI{
    ...
    self.currentIndex = self.lock.wifiDevice.openForce.intValue;
    self.volLevelDataModel = self.lock.wifiDevice.openForce.intValue;
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [[KDSMQTTManager sharedManager] setOpenForceWithWf:self.lock.wifiDevice value:(int)self.currentIndex completion:^(NSError * _Nullable error, BOOL success) {
                    ...
                    if (success) {
                        ...
                        self.lock.wifiDevice.openForce = [NSString stringWithFormat:@"%d",(int)self.currentIndex];
                    }else{
...
-(NSMutableArray *)dataSourceArr{
    if (!_dataSourceArr) {
        _dataSourceArr = [NSMutableArray arrayWithObjects:@"低扭力",@"高扭力", nil];
    }
