...
@implementation PLPLineChartView
        ...
        if (i == self.splitCount) {
            ...
            for (int k = 0; k < self.horizontalDataArr.count; k ++) {
                ...
                if ([tempNum intValue] == 0) {
                    y = (maxMidY - (tempNum.integerValue - self.min.floatValue) * ratio) -1;
                }else{
                    y = maxMidY - (tempNum.integerValue - self.min.floatValue) * ratio;
                }
...
- (void)drawLineLayerWithPointArr:(NSMutableArray *)pointArr maxMidY:(CGFloat)maxMidY
{
    ...
    lineLayer.shadowColor =  [UIColor colorWithRed:91/255.0 green:187/255.0 blue:183/255.0 alpha:0.80].CGColor;
