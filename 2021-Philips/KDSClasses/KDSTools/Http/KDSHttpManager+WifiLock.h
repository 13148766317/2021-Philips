...
/**
 *@abstract 设置胁迫密钥的接收用户
 *@param uid 服务器登录接口成功时返回的uid(管理员用户ID)。
 *@param wifiSN    wifi模块SN
 *@param num 密钥编号
 *@param pwdType 密钥类型：1密码 2指纹密码 3卡片密码 4面容识别
 *@param accountType  胁迫报警接口账户类型。1 手机号；2 邮箱
 *@param duressAlarmAccount 胁迫报警接口账户
 *@param success 请求成功执行的回调。
 *@param errorBlock 出错执行的回调，参数error的code是服务器返回的code，domain是服务器返回的msg。
 *@param failure 由于数据解析及网络等原因失败的回调，error是系统传递过来的。
 *@return 当前的请求任务。
 ...
 *@param uid 服务器登录接口成功时返回的uid(管理员用户ID)。
 *@param wifiSN    wifi模块SN
 *@param duressAlarmSwitch  胁迫开关：0 关闭胁迫报警  1 开启胁迫报警
 *@param success 请求成功执行的回调。
 *@param errorBlock 出错执行的回调，参数error的code是服务器返回的code，domain是服务器返回的msg。
 *@param failure 由于数据解析及网络等原因失败的回调，error是系统传递过来的。
 *@return 当前的请求任务。
...
/**
 *@abstract 删除服务器记录的wifi锁的密码(密码、卡片、指纹)。
 *@param model 密匙信息模型。
 *@param uid 服务器返回的uid。
 *@param wifiSN 设备唯一编号。
 *@param pwdList 设备唯一编号。
 *@param success 请求成功执行的回调。
 *@param errorBlock 出错执行的回调，参数error的code是服务器返回的code，domain是服务器返回的msg。
 *@param failure 由于数据解析及网络等原因失败的回调，error是系统传递过来的。
 *@return 当前的请求任务。
 ...
 *@param startTime  记录开始时间，单位为秒
 *@param endTime 记录结束时间，单位为秒
 *@param markIndex 区分是正常获取操作记录还是按照时间筛选的操作记录    正常获取：1   按时间筛选操作记录：2
...
/// @param serialNumber 门锁eSN号
/// @param customer 客户代号，1：凯迪仕 、2：小凯 、3：桔子物联、 4：飞利浦
/// @param versions   蓝牙版本号
/// @param success 请求成功执行的回调
/// @param errorBlock 出错执行的回调，参数error的code是服务器返回的code，domain是服务器返回的msg
/// @param failure 由于数据解析及网络等原因失败的回调，error是系统传递过来的@return 当前的请求任务
// 检查多固件升级
...
///  门锁当天统计
/// @param uid  用户ID
/// @param wifiSN  wifi锁的SN
/// @param success   成功时的回调
/// @param errorBlock  错误回调
/// @param failure    失败回调;
-(NSURLSessionDataTask *)getwifiLockStatisticsDayWithUid:(NSString *)uid wifiSN:(NSString *)wifiSN success:(nullable void(^)(PLPDoorLockStatisticsModel * model))success error:(void (^)(NSError * _Nonnull))errorBlock failure:(void (^)(NSError * _Nonnull))failure;
...
/// @param errorBlock  错误回调
/// @param failure    失败回调;
-(NSURLSessionDataTask *)getwifiLockStatistics7DaysWithUid:(NSString *)uid wifiSN:(NSString *)wifiSN success:(nullable void(^)(NSArray<PLPWeekStatisticsModel * > *statisticsList))success error:(void (^)(NSError * _Nonnull))errorBlock failure:(void (^)(NSError * _Nonnull))failure;
