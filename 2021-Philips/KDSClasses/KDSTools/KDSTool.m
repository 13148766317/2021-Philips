...
+ (void)setDefaultLoginApple:(NSString *)Apple
{
    [[NSUserDefaults standardUserDefaults] setObject:Apple forKey:@"KDSLoginApple"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
+(NSString *)getDefaultLoginApple
{
    return [[NSUserDefaults standardUserDefaults] objectForKey:@"KDSLoginApple"];
}
+ (void)setDefaultLoginAppleDic:(NSDictionary *)Appledic
{
    [[NSUserDefaults standardUserDefaults] setObject:Appledic forKey:@"KDSLoginAppleDic"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
+(NSDictionary *)getDefaultLoginAppleDic
{
    return [[NSUserDefaults standardUserDefaults] objectForKey:@"KDSLoginAppleDic"];
}
...
+ (NSString *)getTimeFromTimestamp:(long long)time{
...
+(int)timeSwitchTimestamp:(NSString *)time{
...
#pragma mark -用户名显示（显示规则，如果长度大于length.只显示前面是个字符，其余的以...代替）
+(NSString *) showNameStr:(NSString *)nameStr Length:(NSInteger)length{
    
    NSString *endStr = nil;
    if (nameStr.length > length) {
        
        NSString *middleStr = [nameStr substringWithRange:NSMakeRange(0, length)];
        NSString *moreStr = @"...";
        endStr = [NSString stringWithFormat:@"%@%@",middleStr,moreStr];
    }else{
        endStr = nameStr;
    }
...
+ (NSString *)getLocalDateFormateUTCDate:(NSDate *)utcDate{
    ...
    NSString *timeString = [df stringFromDate:utcDate];
    return timeString;
}

#pragma mark ---- 保存ssid与密码

#define  kUserDefaultInputWifiSSID @"kUserDefaultInputWifiSSID"
/**
 * 获取ssid保存过的密码
 */
+(NSString *) getPwdWithSSID:(NSString *) ssid {
    NSString *passwd = nil;
    NSDictionary *dict = [[ NSUserDefaults standardUserDefaults] objectForKey:kUserDefaultInputWifiSSID];
    if (dict && [dict isKindOfClass:[NSDictionary class]] && ssid) {
        passwd = dict[ssid];
    }
    return passwd;
}
 ...
 * 保存ssid与密码
 */
+(void) saveSSID:(NSString *) ssid pwd:(NSString *) pwd {
    if (ssid) {
        NSDictionary *dict = [KDSUserDefaults  objectForKey:kUserDefaultInputWifiSSID];
        NSMutableDictionary *saveDict = nil;
        if (dict && [dict isKindOfClass:[NSDictionary class]] && ssid) {
            saveDict = [[NSMutableDictionary alloc] initWithDictionary:dict];
        }else {
            saveDict = [[NSMutableDictionary alloc] init];
        }
        saveDict[ssid] = pwd ? pwd : @"";
        [KDSUserDefaults setObject:saveDict forKey:kUserDefaultInputWifiSSID];
        [KDSUserDefaults synchronize];
