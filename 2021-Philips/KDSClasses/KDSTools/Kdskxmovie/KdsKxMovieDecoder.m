...
////////////////////////////////////////////////////////////////////////////////
NSString * KdskxmovieErrorDomain = @"ru.kolyvan.kxmovie";
static void FFLog(void* context, int level, const char* format, va_list args);

static NSError * KdskxmovieError (NSInteger code, id info)
{
    ...
    return [NSError errorWithDomain:KdskxmovieErrorDomain
                               code:code
                           userInfo:userInfo];
}

static NSString * errorMessage (KdskxMovieError errorCode)
{
    switch (errorCode) {
...
@interface KdsKxMovieFrame()
@property (readwrite, nonatomic) CGFloat position;
@property (readwrite, nonatomic) CGFloat duration;
@end

@implementation KdsKxMovieFrame
@end

@interface KdsKxAudioFrame()
@property (readwrite, nonatomic, strong) NSData *samples;
@end

@implementation KdsKxAudioFrame
- (KdsKxMovieFrameType) type { return KxMovieFrameTypeAudio; }
@end

@interface KdsKxVideoFrame()
@property (readwrite, nonatomic) NSUInteger width;
@property (readwrite, nonatomic) NSUInteger height;
@end

@implementation KdsKxVideoFrame
- (KdsKxMovieFrameType) type { return KxMovieFrameTypeVideo; }
@end

@interface KdsKxVideoFrameRGB ()
@property (readwrite, nonatomic) NSUInteger linesize;
@property (readwrite, nonatomic, strong) NSData *rgb;
@end

@implementation KdsKxVideoFrameRGB
- (KdsKxVideoFrameFormat) format { return KxVideoFrameFormatRGB; }
- (UIImage *) asImage
{
    UIImage *image = nil;
...
@implementation KdsKxVideoFrameYUV
- (KdsKxVideoFrameFormat) format { return KxVideoFrameFormatYUV; }
@end

@interface KdsKxArtworkFrame()
@property (readwrite, nonatomic, strong) NSData *picture;
@end

@implementation KdsKxArtworkFrame
- (KdsKxMovieFrameType) type { return KxMovieFrameTypeArtwork; }
- (UIImage *) asImage
{
    UIImage *image = nil;
...
@interface KdsKxSubtitleFrame()
@property (readwrite, nonatomic, strong) NSString *text;
@end

@implementation KdsKxSubtitleFrame
- (KdsKxMovieFrameType) type { return KxMovieFrameTypeSubtitle; }
@end

////////////////////////////////////////////////////////////////////////////////

@interface KdsKxMovieDecoder () {
    ...
    NSDictionary        *_info;
    KdsKxVideoFrameFormat  _videoFrameFormat;
    NSUInteger          _artworkStream;
    NSInteger           _subtitleASSEvents;
}
@end

@implementation KdsKxMovieDecoder
...
- (void) setSelectedAudioStream:(NSInteger)selectedAudioStream
{
    ...
    KdskxMovieError errCode = [self openAudioStream: audioStream];
...
- (void) setSelectedSubtitleStream:(NSInteger)selected
{
    ...
    if (selected == -1) {
        ...
        KdskxMovieError errCode = [self openSubtitleStream:subtitleStream];
        if (kxMovieErrorNone != errCode) {
...
{
    KdsKxMovieDecoder *mp = [[KdsKxMovieDecoder alloc] init];
    if (mp) {
            ...
            error: (NSError **) perror
{
    ...
    _path = path;
        //打开文件
    KdskxMovieError errCode = [self openInput: path];
    
    if (errCode == kxMovieErrorNone) {
        
        KdskxMovieError videoErr = [self openVideoStream];//打开视频流
        KdskxMovieError audioErr = [self openAudioStream];//打开音频流
    ...
    if (errCode != kxMovieErrorNone) {
        ...
        if (perror)
            *perror = KdskxmovieError(errCode, errMsg);
        return NO;
    }
...
- (KdskxMovieError) openInput: (NSString *) path
{
...
//音视频的流打开函数
- (KdskxMovieError) openVideoStream
{
    KdskxMovieError errCode = kxMovieErrorStreamNotFound;
    _videoStream = -1;
    _artworkStream = -1;
    //收集视频流
...
- (KdskxMovieError) openVideoStream: (NSInteger) videoStream
{    
    // get a pointer to the codec context for the video stream
    AVCodecContext *codecCtx = _formatCtx->streams[videoStream]->codec;
    
    // find the decoder for the video stream (h264)
...
- (KdskxMovieError) openAudioStream
{
    KdskxMovieError errCode = kxMovieErrorStreamNotFound;
    _audioStream = -1;
...
- (KdskxMovieError) openAudioStream: (NSInteger) audioStream
{   
...
- (KdskxMovieError) openSubtitleStream: (NSInteger) subtitleStream
{
    ...
    if (codecCtx->subtitle_header_size) {
        ...
        if (s.length) {
            ...
            NSArray *fields = [KdsKxMovieSubtitleASSParser parseEvents:s];
...
- (KdsKxVideoFrame *) handleVideoFrame
{
    ...
    if (_videoFrameFormat == KxVideoFrameFormatYUV) {
        ...
        KdsKxVideoFrameYUV * yuvFrame = [[KdsKxVideoFrameYUV alloc] init];
        ...
        KdsKxVideoFrameRGB *rgbFrame = [[KdsKxVideoFrameRGB alloc] init];
...
- (KdsKxAudioFrame *) handleAudioFrame
{
    ...
    KdsKxAudioFrame *frame = [[KdsKxAudioFrame alloc] init];
...
- (KdsKxSubtitleFrame *) handleSubtitle: (AVSubtitle *)pSubtitle
{
    NSMutableString *ms = [NSMutableString string];
        ...
        if (rect) {
            ...
            if (rect->text) { // rect->type == SUBTITLE_TEXT
                ...
                if (s.length) {
                    ...
                    NSArray *fields = [KdsKxMovieSubtitleASSParser parseDialogue:s numFields:_subtitleASSEvents];
                    if (fields.count && [fields.lastObject length]) {
                        
                        s = [KdsKxMovieSubtitleASSParser removeCommandsFromEventText: fields.lastObject];
                        if (s.length) [ms appendString:s];
                    }                    
                }
            }
    ...
    KdsKxSubtitleFrame *frame = [[KdsKxSubtitleFrame alloc] init];
    frame.text = [ms copy];   
...
//                                              _videoCodecCtx->height);
                    ...
                    KdsKxVideoFrame *frame = [self handleVideoFrame];
                    if (frame) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    KdsKxAudioFrame * frame = [self handleAudioFrame];
                    if (frame) {
            ...
            if (packet.size) {
                ...
                KdsKxArtworkFrame *frame = [[KdsKxArtworkFrame alloc] init];
            ...
            while (pktSize > 0) {
                ...
                if (gotsubtitle) {
                    ...
                    KdsKxSubtitleFrame *frame = [self handleSubtitle: &subtitle];
                    if (frame) {
                        [result addObject:frame];
                    }
...
static int interrupt_callback(void *ctx)
{
    if (!ctx)
        return 0;
    __unsafe_unretained KdsKxMovieDecoder *p = (__bridge KdsKxMovieDecoder *)ctx;
...
@implementation KdsKxMovieSubtitleASSParser
