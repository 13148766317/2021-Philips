...
#import "KdsKxMovieViewController.h"
#import <MediaPlayer/MediaPlayer.h>
#import <QuartzCore/QuartzCore.h>
#import "KdsKxMovieDecoder.h"
#import "KdsKxAudioManager.h"
#import "KdsKxMovieGLView.h"
#import "KdsKxLogger.h"
//#import "MainTool.h"

NSString * const KdsKxMovieParameterMinBufferedDuration = @"KdsKxMovieParameterMinBufferedDuration";
NSString * const KdsKxMovieParameterMaxBufferedDuration = @"KdsKxMovieParameterMaxBufferedDuration";
NSString * const KdsKxMovieParameterDisableDeinterlacing = @"KdsKxMovieParameterDisableDeinterlacing";
...
#define LOCAL_MIN_BUFFERED_DURATION   1.0//0.2
#define LOCAL_MAX_BUFFERED_DURATION   2.0//0.4
#define NETWORK_MIN_BUFFERED_DURATION 2.0
#define NETWORK_MAX_BUFFERED_DURATION 4.0

@interface KdsKxMovieViewController () {

    KdsKxMovieDecoder      *_decoder;
    ...
    CGFloat             _minBufferedDuration;//最小缓存时长
    CGFloat             _maxBufferedDuration;//最大缓存时长
    BOOL                _buffered;
...
@property (readwrite, strong) KdsKxArtworkFrame *artworkFrame;
@end

@implementation KdsKxMovieViewController
...
{    
    id<KdsKxAudioManager> audioManager = [KdsKxAudioManager audioManager];
    [audioManager activateAudioSession];    
    return [[KdsKxMovieViewController alloc] initWithContentPath: path parameters: parameters];
}
...
//        self.wantsFullScreenLayout = YES;
        ...
        __weak KdsKxMovieViewController *weakSelf = self;
        
        KdsKxMovieDecoder *decoder = [[KdsKxMovieDecoder alloc] init];
        
        decoder.interruptCallback = ^BOOL(){
            
            __strong KdsKxMovieViewController *strongSelf = weakSelf;
...
- (void)loadView
{
    ...
    CGFloat topH = 50;
    CGFloat botH = 50;
    CGFloat iphonexBuffTop = 25;
    CGFloat iphonexBuffHeight = 20;
//    CGFloat iphonexBuffBottom = 20;
    _topHUD    = [[UIView alloc] initWithFrame:CGRectMake(0,0,0,0)];
    _topBar    = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, width,isFullScreeniPhone?topH+iphonexBuffHeight:topH)];
    _bottomBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, height-botH, width, botH)];
    _bottomBar.tintColor = [UIColor blackColor];

    _topHUD.frame = CGRectMake(0,0,width,isFullScreeniPhone? _topBar.frame.size.height+iphonexBuffTop:_topBar.frame.size.height);
    ...
    _doneButton.frame = CGRectMake(0,isFullScreeniPhone?1+iphonexBuffTop:1, 50,topH);
...
//    [_doneButton setContentVerticalAlignment:UIControlContentVerticalAlignmentCenter];
    ...
    _progressLabel = [[UILabel alloc] initWithFrame:CGRectMake(46, isFullScreeniPhone?1+iphonexBuffTop:1, 50,topH)];
    ...
    _progressSlider = [[UISlider alloc] initWithFrame:CGRectMake(100, isFullScreeniPhone?2+iphonexBuffTop:2, width-197,topH)];
...
//                          forState:UIControlStateNormal];
    ...
    _leftLabel = [[UILabel alloc] initWithFrame:CGRectMake(width-92,isFullScreeniPhone?1+iphonexBuffTop:1, 60, topH)];
    ...
    _infoButton.frame = CGRectMake(width-31, isFullScreeniPhone?(topH-20)/2+1+iphonexBuffTop:(topH-20)/2+1, 20, 20);
...
-(void) play
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){//为了做开始加载的缓存
        [self tick];
    });
...
- (void) setMovieDecoder: (KdsKxMovieDecoder *) decoder
               withError: (NSError *) error
{
    ...
    if (!error && decoder) {
        ...
        if (_parameters.count) {
            ...
            val = [_parameters valueForKey: KdsKxMovieParameterMinBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _minBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: KdsKxMovieParameterMaxBufferedDuration];
            if ([val isKindOfClass:[NSNumber class]])
                _maxBufferedDuration = [val floatValue];
            
            val = [_parameters valueForKey: KdsKxMovieParameterDisableDeinterlacing];
            if ([val isKindOfClass:[NSNumber class]])
...
- (void) setupPresentView
{
    ...
    if (_decoder.validVideo) {
        _glView = [[KdsKxMovieGLView alloc] initWithFrame:bounds decoder:_decoder];
    } 
            ...
            if (!_currentAudioFrame) {
                ...
                @synchronized(_audioFrames) {
                    ...
                    if (count > 0) {
                        ...
                        KdsKxAudioFrame *frame = _audioFrames[0];
...
- (BOOL) addFrames: (NSArray *)frames
{
    if (_decoder.validVideo) {
        ...
        @synchronized(_videoFrames) {
            ...
            for (KdsKxMovieFrame *frame in frames)
        ...
        if (!_decoder.validVideo) {
            ...
            for (KdsKxMovieFrame *frame in frames)
                if (frame.type == KxMovieFrameTypeArtwork)
                    self.artworkFrame = (KdsKxArtworkFrame *)frame;
        }
    }
...
- (void) asyncDecodeFrames
{
    ...
    __weak KdsKxMovieViewController *weakSelf = self;
    __weak KdsKxMovieDecoder *weakDecoder = _decoder;
    ...
    dispatch_async(_dispatchQueue, ^{
        ...
        BOOL good = YES;
        while (good) {//使这个线程一直存活
            
            good = NO;
            
            @autoreleasepool {
                
                __strong KdsKxMovieDecoder *decoder = weakDecoder;
                ...
                if (decoder && (decoder.validVideo || decoder.validAudio)) {
                    ...
                    if (frames.count) {
                        ...
                        __strong KdsKxMovieViewController *strongSelf = weakSelf;
                        if (strongSelf)
                            good = [strongSelf addFrames:frames];//一直在解码数据将解码玩的数据放addFrames进行处理
                    }
                }
            }
        }
...
//        NSLog(@"--{Kaadas}--leftFrames-=_videoFrames=%lu",(unsigned long)_videoFrames.count);
//        NSLog(@"--{Kaadas}--leftFrames-=_audioFrames=%lu", (unsigned long)_audioFrames.count);
//        NSLog(@"--{Kaadas}--leftFrames-=leftFrames=%lu",(unsigned long)leftFrames);
        ...
        if (!leftFrames ||
            !(_bufferedDuration > _minBufferedDuration)) {//开始转
            //解码frame
            [self asyncDecodeFrames];
        }
...
- (CGFloat) tickCorrection
{
    ...
    if (!_tickCorrectionTime) {
        ...
        _tickCorrectionTime = now;//播放的位置 就是现在播的的时间
...
- (CGFloat) presentVideoFrame: (KdsKxVideoFrame *) frame
{
    if (_glView) {
        ...
        KdsKxVideoFrameRGB *rgbFrame = (KdsKxVideoFrameRGB *)frame;
...
- (void) presentSubtitles
{
    ...
    if ([self subtitleForPosition:_moviePosition
        ...
        if (actual.count) {
            ...
            for (KdsKxSubtitleFrame *subtitle in actual.reverseObjectEnumerator) {
                    ...
                    outdated: (NSArray **) pOutdated
{
    ...
    for (KdsKxSubtitleFrame *subtitle in _subtitles) {
...
- (void) updateHUD
{
    ...
    NSLog(@"--{Kaadas}--视频-持续时间=%f",duration);
    NSLog(@"--{Kaadas}--视频-视频位置=%f",_moviePosition);
    NSLog(@"--{Kaadas}--视频-译码器开始时间=%f",_decoder.startTime);
    NSLog(@"--{Kaadas}--视频-position位置=%f",position);
    ...
    if (_decoder.duration != MAXFLOAT)
        ...
        _leftLabel.text = formatTimeInterval(duration - position-1, YES);
