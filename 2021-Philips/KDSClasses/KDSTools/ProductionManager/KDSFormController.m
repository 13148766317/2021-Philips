...
@interface XLFormRowDescriptor(_KDSFormController)
...
@interface XLFormSectionDescriptor(_KDSFormController)

-(BOOL)evaluateIsHidden;

@end

@interface XLFormDescriptor (_KDSFormController)
...
@end
@implementation KDSFormController
...
//    if (self.form.title && self.presentingVC){
//        self.presentingVC.title = self.form.title;
//    }
...
-(void)addObserverToController {
    ...
    [nc addObserver:self.presentingVC
...
-(void)removeObserverFromController {
    NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
    [nc removeObserver:self.presentingVC name:UIContentSizeCategoryDidChangeNotification object:nil];
    [nc removeObserver:self.presentingVC name:UIKeyboardWillShowNotification object:nil];
    [nc removeObserver:self.presentingVC name:UIKeyboardWillHideNotification object:nil];
}
...
-(void)showFormValidationError:(NSError *)error
{
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"OK"
                                                      ...
                                                      handler:nil]];
    [self.presentingVC presentViewController:alertController animated:YES completion:nil];
}
...
{
    UIResponder * responder = [self.presentingVC  targetForAction:selector withSender:sender];;
    if (responder) {
...
-(void)setForm:(XLFormDescriptor *)form
{
    ...
    _form.delegate = self;
    [_form forceEvaluate];
    if ([self.presentingVC isViewLoaded]){
        [self.tableView reloadData];
    }
}
...
-(XLFormRowNavigationAccessoryView *)navigationAccessoryView
{
    ...
    _navigationAccessoryView.tintColor = self.presentingVC.view.tintColor;
